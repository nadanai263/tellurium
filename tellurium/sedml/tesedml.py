...
DataDescription
    External data can be specificed in SED-ML via the DataDescription.
...
Tellurium implements full support for SED-ML L1V3 with SBML as model format.
For CellML models only basic support is implemented. CellML models are converted
to SBML models. This results in different XPath expressions, in the SBML model
so that XPath targets cannot be easily resolved in the CellMl-SBML.
...
XML transformation changes of the model
    ...
    - Change.AddXML
    - Change.ChangeXML
and complex XPath expressions are supported.
...
class SEDMLCodeFactory(object):
    ...
    def taskToPython(doc, task):
        ...
        Tasks contain other subtasks, which can contain subtasks. This results in a tree of task dependencies where the
        simple tasks are the node leaves. These tree has to be resolved to generate code for
        ...
        :return:
        """
        # DataGenerators for the given task
...
class SEDMLTools(object):
    ...
    @classmethod
    def readSEDMLDocument(cls, inputStr, workingDir=None):
        """ Parses SedMLDocument from given input string or path.

        :param inputStr: SED-ML to read, either SED-ML string or path to SED-ML file
        :param workingDir: working directory relative to which the SED-ML resources are resolved.
        ...
        # SEDML-File
            ...
            if not os.path.isfile(inputStr):
                raise IOError("SED-ML path is not a file:", inputStr)

            filename, extension = os.path.splitext(os.path.basename(inputStr))
            if extension not in [".sedml", '.xml']:
                warnings.warn("SEDML file should have [.sedml|.xml] extension:", inputStr)
